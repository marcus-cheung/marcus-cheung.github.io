...
const MoleFollower = ({themeIndex}) => {
    ...
    function walk(character: any, start: Coordinate, end: Coordinate, speed: number = 100) {
        ...
        spriteMaster.add(flip)
        // Sprite loop
        const spriteLoop = gsap.timeline({repeat: -1, delay: 0.15})
                        .to(character, {backgroundPositionX: `${spriteDim.W * (numWalkFrames - 1)}px`, ease: `steps(${numWalkFrames - 1})`, duration: 0.25});
        spriteMaster.add(spriteLoop)
        spriteMaster.play();
        // Movement
        const move = gsap.timeline({overwrite: 'auto'})
                    .to(character, {x: end.X,
                                    ...
                                    delay: 0.2,
                                    onComplete: function() {spriteMaster.pause(); setWalking(false); stand(character); }});
        move.play();
    }
    ...
    useEffect(() => {
        ...
        const onMouseMove = (event) => {
            ...
            if (!locked) {
                // Kill animations
                gsap.killTweensOf(character);
                if (!grounded && !inBounds(mouseCoords, characterRect)) {
