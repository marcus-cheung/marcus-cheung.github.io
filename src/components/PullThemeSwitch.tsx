...
const PullThemeSwitch = function (props: Props){
  ...
  const sceneHeight = thresh * 1.5;
    ...
    useEffect(() => {
        ...
        var topBlock = Bodies.rectangle(sceneWidth/2, 0, 10, 10, { collisionFilter: { group: group}, isStatic: true, render: {fillStyle: '#808080'}})
    
        var bottomBall = Bodies.circle(sceneWidth/2, 40, 10, { collisionFilter: { group: group}, render: {fillStyle: '#808080'}})
    
        var rope = Composites.stack(sceneWidth/2, 0, 15, 1, 10, 0, function(x, y) {
        ...
        const topConstraint = Constraint.create({
            ...
            bodyB: rope.bodies[0],
            stiffness: 0,
            length: 2, collisionFilter: { group: group},
            render: {true: false}
        });
        ...
        const bottomConstraint = Constraint.create({
            ...
            stiffness: 0,
            length: 0.5, collisionFilter: { group: group}
        });
        ...
        const onDown = (event) => {
            ...
            if (event.clientX >= bottomBall.bounds.min.x + rect.left - 20 &&
                event.clientX <= bottomBall.bounds.max.x + rect.left + 20 &&
                event.clientY >= bottomBall.bounds.min.y + rect.top - 20 &&
                event.clientY <= bottomBall.bounds.max.y + rect.top + 20) {
